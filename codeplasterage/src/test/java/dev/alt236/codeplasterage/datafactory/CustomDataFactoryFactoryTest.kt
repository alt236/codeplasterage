...
internal class CustomDataFactoryFactoryTest {
    ...
    @Suppress("unused")
    private class FactoryWithCorrectConstructor(
        val boolean: Boolean,
    ) : SubDataFactory(true) {
        override fun canCreateDataFor(clazz: Class<*>): Boolean = true

        override fun getDummyData(clazz: Class<*>): DataFactoryResult<*> = DataFactoryResult.Error(IllegalStateException(), clazz)
    }
    ...
    private class FactoryWithWrongConstructor_NonBoolean(
